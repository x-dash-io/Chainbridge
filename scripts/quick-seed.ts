...
async function generateUsers() {
  ...
  for (const role of ROLES) {
    let usersCreated = 0;
    let attempts = 0;
    const maxAttempts = USERS_PER_ROLE * 5; // Prevent infinite loop
    
    while (usersCreated < USERS_PER_ROLE && attempts < maxAttempts) {
      attempts++;
      ...
      try {
        ...
        if (error.code === '23505') { // Unique constraint violation
          continue; // Try different name combination
        }
        throw error;
      }
    }
    
    if (usersCreated < USERS_PER_ROLE) {
      console.log(`Warning: Could only create ${usersCreated}/${USERS_PER_ROLE} ${role}s due to existing data`);
    }
  }
...
async function seedDatabase() {
  ...
  try {
    // Generate users (will use existing if found)
    const createdUsers = await generateUsers();
    
    // Get all users (including existing ones) for the seed process
    const allUsers = await db.select().from(users);
    
    // Map users by role
    const userMap = new Map<string, any[]>();
    for (const role of ROLES) {
      userMap.set(role, allUsers.filter((u: any) => u.role === role));
    }
    
    // Check if we have enough users for each role
    for (const role of ROLES) {
      const roleUsers = userMap.get(role) || [];
      if (roleUsers.length < USERS_PER_ROLE) {
        console.log(`Warning: Only ${roleUsers.length} ${role}s available (needed ${USERS_PER_ROLE})`);
      }
    }
